...
> Przeczytaj `AGENTS.md` - pełne reguły projektu (source of truth).
> Ten plik jest ładowany automatycznie przez Claude Code (w tym przez każdego teammate w Agent Teams).
...
Jeśli musisz zmienić plik innego teammate - wyślij mu wiadomość.

## Subagenci

Definicje subagentów w `.claude/agents/`. Każdy odwołuje się do pełnej roli z `.agents/roles/`.

| Subagent | Plik roli | Tools |
|---|---|---|
...
| /audit | Pełny code audit (quality-gate) |
| /review | Szybki code review |

## Skills

Skills w `.agents/skills/<nazwa>/SKILL.md` (folder-based, format Anthropic).
Ładowane automatycznie przez Claude Code (via symlink `.claude/skills`).

## Komunikacja między agentami

### Raporty (file-based, przetrwają zamknięcie sesji)

| Typ raportu | Plik | Kto pisze | Kto czyta |
|---|---|---|---|
...
- Skończyłeś swój task -> message następnego w kolejce
- API contract się zmienił -> message frontend-engineer
- Znalazłeś blocker -> message lead

NIE wysyłaj broadcast bez potrzeby - każda wiadomość zużywa kontekst.
Ważne decyzje zapisuj w plikach (przetrwają compaction).

### Cykl implementacyjny

```
Architect: analiza -> arch-*.md -> message backend-engineer z wytycznymi
...
Quality Gate: review -> review-*.md
  ...
  FAIL -> message backend-engineer z listą poprawek -> engineer poprawia -> nowy cykl
```
